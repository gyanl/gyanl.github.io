...
**The home work grid is hardcoded in `_includes/home/work.html`** — a `work_data` string of `title|description|thumbnail|url` cards split on `;;`, in a hand-chosen order. It is no longer tag-driven: the project posts sit in `_posts/Drafts/` (excluded from the build) until their case studies are written, so every card currently links out to the live site. When a case study is published, point its card at the post URL and drop the `target`/`rel` and the arrow badge. The `featured`/`featured_order` front matter on those posts is inert.
